    ...
    if(format == FORMAT_LDR) {
        for(int i = 0; i < height / 2; i++) {
            for(int j = 0; j < width; j++) {
                uchar tmp;
                int p1 = (i * width + j) * channels, p2 = ((height - i - 1) * width + j) * channels;
    ...
    if(format == FORMAT_LDR) {
        for(int i = 0; i < height; i++) {
            for(int j = 0; j < width / 2; j++) {
                uchar tmp;
                int p1 = (i * width + j) * channels, p2 = (i * width + width - j - 1) * channels;
                for(int k = 0; k < channels; k++) {
                    tmp = ldr_buffer[p1 + k];
                    ldr_buffer[p1 + k] = ldr_buffer[p2 + k];
                    ldr_buffer[p2 + k] = tmp;
...
int image_t::get_channels() const {
    return channels;
}

format_t image_t::get_format() const {
    return format;
}
